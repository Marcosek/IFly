...
<body>
  ...
  if(isset($_POST['submit']))
  {
    ...
    $query = "SELECT flights.*, planes.name AS plane_name, planes.plane_id AS id_plane, planes.status AS plane_status, 
    tour.departure_location,  tour.arrival_location, tour.business_class, tour.economy_class
          FROM flights
...
if ($dataod) {
    $nextDay = date('Y-m-d', strtotime($dataod . ' + 1 day'));
    $query .= " AND flights.start > '$dataod' AND flights.start < '$nextDay'";
}
...
echo "<table border='1'>";
echo "<tr><th>" . ($rodzajLotu ? "odloty" : "przyloty") . "</th><th>Kierunek</th><th>Numer Lotu</th><th>" . ($rodzajLotu ? "Terminal" : "Gate") . "</th><th>Biznes klasa</th><th>Ekonomiczna klasa</th><th>Kup bilet</th></tr>";
while ($row = $result->fetch_assoc()) {
    echo "<tr>";
            ...
            echo "<td>".$row['business_class']."</td>";
            echo "<td>".$row['economy_class']."</td>";
            
            echo "<td><a href='kup_bilet.php?flight_id=".$row['ID']."'>Kup bilet</a></td>";
